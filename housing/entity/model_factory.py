...
def evaluate_regression_model(model_list: list, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy:float=0.6) -> MetricInfoArtifact:
    ...
    try:
        ...
        for model in model_list:
            ...
            lg.info(f"{'>>'*30}Started evaluating model: [{type(model).__name__}] {'<<'*30}")
            ...
            lg.info(f"{'>>'*30} Score {'<<'*30}")
            lg.info(f"Train Score\t\t Test Score\t\t Average Score")
            lg.info(f"{train_acc}\t\t {test_acc}\t\t{model_accuracy}")

            lg.info(f"{'>>'*30} Loss {'<<'*30}")
            lg.info(f"Diff test train accuracy: [{diff_test_train_acc}].") 
            lg.info(f"Train root mean squared error: [{train_rmse}].")
            lg.info(f"Test root mean squared error: [{test_rmse}].")
            ...
            if model_accuracy >= base_accuracy and diff_test_train_acc < 0.05:
                ...
                lg.info(f"Acceptable model found {metric_info_artifact}. ")
            index_number += 1
        if metric_info_artifact is None:
            lg.info(f"No model found with higher accuracy than base accuracy")
...
class ModelFactory:
    ...
    def update_property_of_class(instance_ref:object, property_data: dict):
        try:
            ...
            for key, value in property_data.items():
                lg.info(f"Executing:$ {str(instance_ref)}.{key}={value}")
        ...
        try:
            ...
            for grid_searched_best_model in grid_searched_best_model_list:
                if base_accuracy < grid_searched_best_model.best_score:
                    lg.info(f"Acceptable model found:{grid_searched_best_model}")
                    base_accuracy = grid_searched_best_model.best_score

                    best_model = grid_searched_best_model
            if not best_model:
                raise Exception(f"None of Model has base accuracy: {base_accuracy}")
            lg.info(f"Best model: {best_model}")
            return best_model
        except Exception as e:
            raise HousingException(e, sys) from e
    ...
    def get_best_model(self, X, y,base_accuracy=0.6) -> BestModel:
        try:
            lg.info("Started Initializing model from config file")
            initialized_model_list = self.get_initialized_model_list()
            lg.info(f"Initialized model: {initialized_model_list}")
